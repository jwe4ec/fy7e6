# ---------------------------------------------------------------------------- #
# RJAGS Models for Treatment Dropout: CBM vs. Psychoeducation v1.12
...
a2_1 <- rep(NA, length(unique(tx_dropout_dat$participantID)))
...
for (i in 1:length(unique(tx_dropout_dat$participantID))) {
  ...
  a1[i] <-
    tx_dropout_dat$a1[which(tx_dropout_dat$participantID ==
                                    unique(tx_dropout_dat$participantID)[i])][1]
  a2_1[i] <-
    tx_dropout_dat$a2_1[which(tx_dropout_dat$participantID ==
                                    unique(tx_dropout_dat$participantID)[i])][1]
}

# Specify JAGS model with likelihood, priors, and parameters of interest ----

modelString_tx_dropout_a2_1 = "model {
  ...
  para[1] <- 2*(beta[14] + beta[15])       # CBM vs. psychoed slope at S2
...
writeLines(modelString_tx_dropout_a2_1,
           con = paste0(path1, "/modelString_tx_dropout_a2_1.txt"))
...
model_tx_dropout_a2_1 <-
  jags.model(file = paste0(path1, "/modelString_tx_dropout_a2_1.txt"),
...
time0 <- proc.time()
update(model_tx_dropout_a2_1, n.iter = burn_iterations)
(burn_in_time <- proc.time() - time0)

time0 <- proc.time()
model.samples_tx_dropout_a2_1 <-
  coda.samples(model_tx_dropout_a2_1,
               c("para"),
...
save(model.samples_tx_dropout_a2_1,
     file = paste0(path1, path2, "/model.samples_tx_dropout_a2_1.RData"))
load(paste0(path1, path2, "/model.samples_tx_dropout_a2_1.RData"))

model.res_tx_dropout_a2_1 <-
  as.mcmc(do.call(rbind, model.samples_tx_dropout_a2_1))

# Save plots to directory ----

pdf(file = paste0(path1, path2, "/plots.pdf"))
par(mfrow=c(4,2))
plot(model.res_tx_dropout_a2_1)
dev.off()
...
print("Summary:")
summary(model.res_tx_dropout_a2_1)

print("HPD Credible Intervals:")
cat("\n")
HPDinterval(model.res_tx_dropout_a2_1)
cat("\n")

print("Geweke's convergence diagnostic:")
(geweke <- geweke.diag(model.res_tx_dropout_a2_1))
