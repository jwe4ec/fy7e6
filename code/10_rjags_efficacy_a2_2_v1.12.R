# -----------------------------------------------------------------------------#
# RJAGS Models for Negative BBSIQ and OASIS: High Risk CBM + Telecoaching vs.
...
a2_2 <- rep(NA, length(unique(bbsiq_dat$participantID)))
...
for (i in 1:length(unique(bbsiq_dat$participantID))) {
  ...
  a1[i] <-
    bbsiq_dat$a1[which(bbsiq_dat$participantID ==
                         unique(bbsiq_dat$participantID)[i])][1]
  a2_2[i] <-
    bbsiq_dat$a2_2[which(bbsiq_dat$participantID ==
                           unique(bbsiq_dat$participantID)[i])][1]
}

# Specify JAGS model with likelihood, priors, and parameters of interest ----

modelString_bbsiq_neg_a2_2 = "model {
  ...
  para[2] <- 2*(beta[31] + beta[34])  # HR+T vs. HR+0 slope at S3
...
writeLines(modelString_bbsiq_neg_a2_2,
           con = paste0(path1, "/model_bbsiq_neg_a2_2.txt"))
...
model_bbsiq_neg_a2_2 <-
  jags.model(file = paste0(path1, "/model_bbsiq_neg_a2_2.txt"),
...
time0 <- proc.time()
update(model_bbsiq_neg_a2_2, n.iter = burn_iterations)
(burn_in_time <- proc.time() - time0)

time0 <- proc.time()
model.samples_bbsiq_neg_a2_2 <-
  coda.samples(model_bbsiq_neg_a2_2,
               c("para"),
...
save(model.samples_bbsiq_neg_a2_2,
     file = paste0(path1, path2, "/model.samples_bbsiq_neg_a2_2.RData"))
load(paste0(path1, path2, "/model.samples_bbsiq_neg_a2_2.RData"))

model.res_bbsiq_neg_a2_2 <-
  as.mcmc(do.call(rbind, model.samples_bbsiq_neg_a2_2))

# Save plots to directory ----

pdf(file = paste0(path1, path2, "/plots.pdf"))
par(mfrow=c(4,2))
plot(model.res_bbsiq_neg_a2_2)
dev.off()
...
print("Summary:")
summary(model.res_bbsiq_neg_a2_2)

print("HPD Credible Intervals:")
cat("\n")
HPDinterval(model.res_bbsiq_neg_a2_2)
cat("\n")

print("Geweke's convergence diagnostic:")
(geweke <- geweke.diag(model.res_bbsiq_neg_a2_2))
...
a2_2 <- rep(NA, length(unique(oasis_dat$participantID)))
...
for (i in 1:length(unique(oasis_dat$participantID))) {
  ...
  a1[i] <-
    oasis_dat$a1[which(oasis_dat$participantID ==
                         unique(oasis_dat$participantID)[i])][1]
  a2_2[i] <-
    oasis_dat$a2_2[which(oasis_dat$participantID ==
                           unique(oasis_dat$participantID)[i])][1]
}

# Specify JAGS model with likelihood, priors, and parameters of interest ----

modelString_oasis_a2_2 = "model {
...
writeLines(modelString_oasis_a2_2,
           con = paste0(path1, "/modelString_oasis_a2_2.txt"))
...
model_oasis_a2_2 <-
  jags.model(file = paste0(path1, "/modelString_oasis_a2_2.txt"),
             data = jagsdata, inits = inits, n.chains = 1)
...
time0 <- proc.time()
update(model_oasis_a2_2, n.iter = burn_iterations)
(burn_in_time <- proc.time() - time0)

time0 <- proc.time()
model.samples_oasis_a2_2 <-
  coda.samples(model_oasis_a2_2,
               c("para"),
               n.iter = remaining_iterations)
(sampling_time <- proc.time() - time0)
...
save(model.samples_oasis_a2_2,
     file = paste0(path1, path2, "/model.samples_oasis_a2_2.RData"))
load(paste0(path1, path2, "/model.samples_oasis_a2_2.RData"))

model.res_oasis_a2_2 <-
  as.mcmc(do.call(rbind, model.samples_oasis_a2_2))

# Save plots to directory ----

pdf(file = paste0(path1, path2, "/plots.pdf"))
par(mfrow=c(4,2))
plot(model.res_oasis_a2_2)
dev.off()
...
print("Summary:")
summary(model.res_oasis_a2_2)

print("HPD Credible Intervals:")
cat("\n")
HPDinterval(model.res_oasis_a2_2)
cat("\n")

print("Geweke's convergence diagnostic:")
(geweke <- geweke.diag(model.res_oasis_a2_2))
print("Parameters in (-1.96, 1.96):")
cat("\n")
print(geweke$z > -1.96 & geweke$z < 1.96)
cat("\n")
