...
boot_start <- as.integer(cmdArgs[2])
boot_stop <- as.integer(cmdArgs[3])
...
myRank <- comm.rank()
numProcs <- comm.size()  #Gets the total number of workers
...
run_jags_model <- function(analysis_type, bs_sample, analysis_sample,
  ...
  comm.print(paste0("Analysis Type: ", analysis_type))
  cat("\n")

  if (!is.null(bs_sample)) {
    comm.print(paste0("Bootstrap Sample: ", bs_sample))
    cat("\n")
  }

  comm.print(paste0("Analysis Sample: ", analysis_sample))
  comm.print(paste0("Contrast: ", a_contrast))
  comm.print(paste0("Outcome: ", y_var))
  cat("\n")

  comm.print(paste0("Total Iterations: ", total_iterations))
  comm.print(paste0("Burn Iterations: ", burn_iterations))
  comm.print(paste0("Remaining Iterations: ", remaining_iterations))
  cat("\n")

  comm.print("Model Creation Time (seconds):")
  comm.print(create_model_time)
  cat("\n")
  comm.print("Burn-In Time (seconds):")
  comm.print(burn_in_time)
  cat("\n")
  comm.print("Sampling Time (seconds):")
  comm.print(sampling_time)
  cat("\n")
  comm.print("Total Time (seconds):")
  ...
  comm.print("HPD Credible Intervals:")
  cat("\n")
  hpd_interval <- HPDinterval(model_res)
  comm.print(hpd_interval)
  cat("\n")

  comm.print("Geweke's convergence diagnostic:")
  geweke <- geweke.diag(model_res)
  comm.print(geweke)
  comm.print("Parameters in (-1.96, 1.96):")
  cat("\n")
  geweke_converge_each <- geweke$z > -1.96 & geweke$z < 1.96
  comm.print(geweke_converge_each)
  cat("\n")
  geweke_converge_all <- all(geweke_converge_each)
  comm.print(paste0("All parameters in (-1.96, 1.96): ", geweke_converge_all))
...
# Run analysis based on ":wa_contrast". "workerNum" (starting at 1) is the bootstrap 
...
if (a_contrast == "a1") {
  ...
  # Set up arrays with sample numbers for bootstrap
  #bootstrap_vector <- 1:(2*numProcs)
  bootstrap_vector <- boot_start:boot_stop
  chunks <- split(bootstrap_vector, cut(seq_along(bootstrap_vector), numProcs, labels=FALSE))
  ...
  results_list <- foreach( bs_sample_num = myChunk ) %do% {
      ...
      run_jags_model(analysis_type, bs_sample = bs_sample_num, analysis_sample,
                                 jags_dat, inits,
                                 a_contrast, y_var, total_iterations)
  }

  names(results_list) <- myChunk
  ...
  bootNum <- workerNum + as.integer((boot_start - 1)/2) 
  outfile <- paste0(out_path, "/results_",bootNum,".RData")
...
#results_all <- unlist(gather(results_list))




# Save results
#save(results_all, file = paste0(model_results_path_stem, "/results.RData"))
